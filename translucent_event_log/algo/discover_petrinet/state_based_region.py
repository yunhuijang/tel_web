...
from translucent_event_log.objects.automaton import transition_system as ts
...
def split_labels(auto, e):
    ...
    if not flag:
        ger = find_GER(auto, e)
        if len(inter) > 0:
            s = inter.difference(ger)
            pset = set(powerset(s))
            for i in pset:
                se = set(i)
                cand_set.add(frozenset(se.union(ger)))
    ...
    for i in cand_set:
        if len(i) > 0:
            ...
            for key, value in dic.items():
                if value not in {1, 10, 11, 100, 1000}:
                    split_event_set.add(key)
            ...
            #     sum += j
            #dict_name = str(i)
            if len(split_event_set) > 0:
                cost_dic[i] = {}
                cost_dic[i]['cost'] = len(split_event_set)
                cost_dic[i]['split'] = split_event_set

    #split_re: proper subset including GER
    #split_act: activity that needs to be splitted
    split_re = min(cost_dic.items(), key = lambda x: x[1]['cost'])[0]
    split_act = min(cost_dic.items(), key = lambda x: x[1]['cost'])[1]['split'] #find the minimum set that violates region
    no_cross = {}
    exit_ = {}
    enter = {}
    ...
    for trans in split_act:
        for tt in newauto.transitions:
            for t in exit_[trans]:
                if t == tt:
                    changed_tt = ts.TransitionSystem.Transition(tt.name + "#", tt.from_state, tt.to_state)
                    newauto.transitions.add(changed_tt)
                    tt.from_state.outgoing.add(changed_tt)
                    tt.to_state.incoming.add(changed_tt)
                    tt.from_state.outgoing.remove(tt)
                    tt.to_state.incoming.remove(tt)
                    newauto.transitions.remove(tt)

            for t in no_cross[trans]:
                if t == tt:
                    changed_tt = ts.TransitionSystem.Transition(tt.name + "!", tt.from_state, tt.to_state)
                    newauto.transitions.add(changed_tt)
                    tt.from_state.outgoing.add(changed_tt)
...
def map_to_PN(auto, region_set):
    ...
    final_state_set = set()
    for state in auto.states:
        if len(state.incoming) < 1:
            initial_state_set.add(state)
        if len(state.outgoing) < 1:
            final_state_set.add(state)
    ...
    for region in region_set:
        ...
        for event, value in is_region(auto, region)[1].items():
            if value == 100: # exit
                ...
                enter_set.add(event)
        if len(exit_set) > 0 or len(enter_set) > 0:
            place = PetriNet.Place(str(region)) #add places to petri net
        ...
        for is_ in initial_state_set:
            if is_ in region:
                initial_marking[place] = 1
                break
        for fs in final_state_set:
            if fs in region:
    ...
    for trans in net.transitions:
        for char in remove_char:
            trans.label = trans.label.replace(char, "")

    im = initial_marking
    fm = final_marking

    # if you want to have final marking region with final states
    # if len(fm) < 1:
    #     fm = final_marking
...
def petri_net_synthesis(auto):
    ...
    while split: #label splitting
        ...
        for e in get_name(newauto.transitions):
            if not is_excitation_closure(newauto, e)[0]:
                newauto = split_labels(newauto, e)
                split = True
                break
    ...
    # if sol is not None: #remove redundant covers
    #     for reg in region_set.copy():
    #         if not sol[reg]:
    #             region_set.remove(frozenset(reg))
